...
int main() {
    printf("=== 10 MILLION KEY PERFORMANCE TEST ===\n\n");
    
    remove("ten_million_test.db");
    initBPTree("ten_million_test.db");
    
    // Test 1: Insert 10M keys
    printf("[1] Inserting 10,000,000 keys...\n");
    ...
    printf("    ✓ Inserted 10,000,000 keys in %.2f seconds\n", insert_time);
    printf("    ✓ Throughput: %.0f inserts/sec\n\n", 10000000.0 / insert_time);
    ...
    start = clock();
    int n2;
    unsigned char** results2 = readRangeData(5000000, 5010000, &n2);
    double range2_time = (double)(clock() - start) / CLOCKS_PER_SEC;
    printf("    Range [5000000-5010000]: %d results in %.3f sec\n", n2, range2_time);
    ...
    printf("SUMMARY:\n");
    printf("  Insert 10M keys:  %.2f sec (%.0f ops/sec)\n", insert_time, 10000000.0 / insert_time);
    printf("  Read 100K keys:   %.2f sec (%.0f ops/sec)\n", read_time, 100000.0 / read_time);
    printf("  Database size:    ~%.2f GB\n", (10000000.0 * 4096 / 39) / (1024*1024*1024));
